defmodule SCADAMaster.Modbus.Client do
  ...
  # GenServer Callbacks
  def init(%{ip: ip}) do
    case :gen_tcp.connect(ip, Modbus.Tcp.port, [:binary, {:active, false}],2000) do
        {:ok, socket} -> {:ok, socket}
        {:error, reason} -> {:ok, "error_connection", @read_timeout}
    end
    #{:ok, socket} = :gen_tcp.connect(ip, Modbus.Tcp.port, [:binary, {:active, false}],2000)
    #{:ok, socket}
  end
